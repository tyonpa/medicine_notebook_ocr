...
class DEIMCriterion(nn.Module):
    ...
    ##追加
    def loss_char_count(self, outputs, targets, indices, num_boxes):
        """
        予測された文字数(pred_char_count) と正解文字数(char_count) の L1 損失を計算。
        targets の各要素に char_mask==False (0) の場合はスキップします。
        
        Args:
            outputs: モデルの出力 dict。'pred_char_count': Tensor[batch, num_queries]
            targets: list of dict。各 dict に 'char_count': Tensor[N] および
                     'char_mask': Tensor[N] が含まれる。
            indices: ハンガリアンマッチングの結果 [(batch_idx, src_idx_list), (batch_idx, tgt_idx_list)]
            num_boxes: 全真値ボックス数の合計 (正規化用)
        
        Returns:
            辞書 {"loss_char_count": Tensor} または、計算不要時は空辞書 {} を返す
        """
        #assert "pred_char_count" in outputs, "モデル出力に pred_char_count がありません"
        if not "pred_char_count_logit" in outputs:
            return {}
        # (1) マッチングインデックスを取得
        src_idx, tgt_idx = self._get_src_permutation_idx(indices), self._get_tgt_permutation_idx(indices)
        # src_idx / tgt_idx はそれぞれ長さ M のタプル(batch_indices, element_indices)

        # (2) マッチした要素ごとに文字数予測と正解を収集
        src_chars = []
        tgt_chars = []
        for (b_src, i_src), (b_tgt, i_tgt) in zip(zip(*src_idx), zip(*tgt_idx)):
            # マスク判定: char_mask[i_tgt]==True のものだけ扱う
            #if b_tgt>= len(targets) or i_tgt >=len(targets[b_tgt]["char_mask"]):
            if i_tgt >=len(targets[b_tgt]["char_count"]):
                continue
            # 予測値・正解値を取り出してリストに追加
            src_chars.append(outputs["pred_char_count_logit"][b_src, i_src])
            tgt_chars.append(targets[b_tgt]["char_count"][i_tgt])

        # (3) マスクで生き残った要素がなければ、損失計算不要
        if len(src_chars) == 0:
            return {}

        # (4) Tensor にまとめて L1 Loss 計算
        src_char_tensor = torch.stack(src_chars, dim=0)  # [K]
        tgt_char_tensor = torch.stack(tgt_chars, dim=0)  # [K]
        #print(src_char_tensor)
        #print(tgt_char_tensor)
        char_num_classes=3
        tgt_char_score = F.one_hot(tgt_char_tensor, num_classes=char_num_classes+1).to(src_char_tensor.dtype)
        #print(tgt_char_score)
        pred_char_score = F.sigmoid(src_char_tensor)
        #print(pred_char_score)
        loss_per_elem = F.binary_cross_entropy(pred_char_score,tgt_char_score,reduction="none")
        #print(tgt_char_tensor)
        #loss_per_elem=F.cross_entropy(src_char_tensor,tgt_char_tensor,reduction="none")
        #loss_char = loss_per_elem.sum() / src_char_tensor.size(0)
        #loss_per_elem = F.l1_loss(src_char_tensor, tgt_char_tensor, reduction="none")  # [K]
        loss_char = loss_per_elem.sum() / src_char_tensor.size(0)
        return {"loss_char_count": loss_char}
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            'char_count':self.loss_char_count
        }
