...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, hidden_dim, decoder_layer, decoder_layer_wide, num_layers, num_head, reg_max, reg_scale, up,
                 eval_idx=-1, layer_scale=2, act='relu', char_head=None):
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            if self.training or i == self.eval_idx:
                ...
                # --- ここから追加: 文字数回帰 ---
                char_scores = char_head[i](output)
                char_pred = self.lqe_layers[i](char_scores, pred_corners)
                #char_pred = char_head[i](output).squeeze(-1)  # [batch, num_queries]
                dec_out_chars.append(char_pred)
                # --- ここまで追加 ---
...
class DFINETransformer(nn.Module):
    ...
    def __init__(self,
      ...
      # denoising
        ...
        # --- ここから追加: 文字数回帰ヘッド ---
        char_num_classes=4
        #self.enc_char_head = nn.Linear(hidden_dim, char_num_classes)
        self.dec_char_head = nn.ModuleList(
            [nn.Linear(hidden_dim, char_num_classes) for _ in range(self.eval_idx + 1)]
          + [nn.Linear(scaled_dim, char_num_classes) for _ in range(num_layers - self.eval_idx - 1)])
        #for m in self.dec_char_head:
    ...
    def forward(self, feats, targets=None):
        ...
        # decoder
        out_bboxes, out_logits, out_corners, out_refs, out_chars, pre_bboxes, pre_logits = self.decoder(
            init_ref_contents,
        ...
        if self.training and dn_meta is not None:
            ...
            dh_out_chars, out_chars = torch.split(out_chars, dn_meta['dn_num_split'], dim=2)  #追加
            dn_out_refs, out_refs = torch.split(out_refs, dn_meta['dn_num_split'], dim=2)


        if self.training:
            out = {'pred_logits': out_logits[-1], 'pred_boxes': out_bboxes[-1], 'pred_corners': out_corners[-1],
                   'ref_points': out_refs[-1],'pred_char_count_logit': out_chars[-1], 'up': self.up, 'reg_scale': self.reg_scale}
        else:
            out = {'pred_logits': out_logits[-1], 'pred_boxes': out_bboxes[-1], 'pred_char_count_logit': out_chars[-1]}
    ...
    def _set_aux_loss2(self, outputs_class, outputs_coord, outputs_corners, outputs_ref,
        ...
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        return [{'pred_logits': a, 'pred_boxes': b, 'pred_corners': c, 'pred_char_count_logit': ch, 'ref_points': d,
                     'teacher_corners': teacher_corners, 'teacher_logits': teacher_logits}
                for a, b, c, d, ch in zip(outputs_class, outputs_coord, outputs_corners, outputs_ref, outputs_char)]
