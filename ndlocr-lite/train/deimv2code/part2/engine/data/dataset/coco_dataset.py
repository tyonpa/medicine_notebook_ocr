...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image: Image.Image, target, **kwargs):
        ...
        ### 追加: 文字数フィールドをテンソル化して取得
        #char_counts = None
        #if anno and "char_count" in anno[0]:
        #    char_counts = torch.tensor([obj["char_count"] for obj in anno], dtype=torch.float32)
        # ここから追加: 各アノテーションごとに char_count の有無をチェックしてマスクを作成
        char_counts = []
        char_masks  = []
        for obj in anno:
            if "char_count" in obj:
                # 文字数情報がある
                if 1<=obj["char_count"]<=20:
                    char_counts.append(3)
                elif 20<obj["char_count"]<=40:
                    char_counts.append(2)
                elif 40<obj["char_count"]<=100:
                    char_counts.append(1)
                else:
                    char_counts.append(0)
                char_masks.append(1)
            else:
                # 文字数情報がない → ダミー値を入れてマスクで弾く
                char_counts.append(1)
                char_masks.append(0)
        # テンソル化
        char_counts = torch.tensor(char_counts, dtype=torch.int64)  # [N]
        char_masks  = torch.tensor(char_masks,  dtype=torch.bool)     # [N]
        ...
        #    target["char_count"] = char_counts  # 各ボックスに対応する文字数テンソルを追加
        target["char_count"] = char_counts
        target["char_mask"]  = char_masks
        if self.return_masks:
            target["masks"] = masks
