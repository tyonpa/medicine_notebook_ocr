...
class RandomRotation90(T.Transform):
    ...
    def forward(self, *inputs):
        inputs_flat = inputs if len(inputs) > 1 else inputs[0]
        if torch.rand(1) < self.p/2:
            # 時計回りに90度回転
            return T.RandomRotation(degrees=(90, 90), expand=True)(inputs_flat)
        elif torch.rand(1) < self.p:
            # 反時計回りに90度回転
            return T.RandomRotation(degrees=(270, 270), expand=True)(inputs_flat)
        return inputs_flat
@register()
